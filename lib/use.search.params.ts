...
type SupportedType = StringConstructor | NumberConstructor | BooleanConstructor | DateConstructor;
...
type UseUrlSearchParamsResults = [InitialType, (nextQuery: InitialType) => void];
...
function isNoneEmptyPrimitiveArray(input: unknown[]) {
  return (
    Array.isArray(input) &&
    input.length > 0 &&
